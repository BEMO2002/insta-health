...
const MedicalConsultationReservationsDetails = () => {
  ...
  useEffect(() => {
    const fetchReservation = async () => {
      ...
      try {
        setLoading(true);
        setError("");
        const res = await baseApi.get(`/MedicalConsultationReservations/${merchantOrderId}`);
        if (res.data?.success && res.data?.data) {
          setReservation(res.data.data);
          
          // Fetch doctor details if doctorId exists
          if (res.data.data.doctorId) {
            try {
              const doctorRes = await baseApi.get(`/Doctors/${res.data.data.doctorId}`);
              if (doctorRes.data?.success && doctorRes.data?.data) {
                setDoctor(doctorRes.data.data);
              }
            } catch (doctorErr) {
              console.error('Error fetching doctor details:', doctorErr);
              // Don't set error, just continue without doctor details
            }
          }
        } else {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const response = await baseApi.post(`/MedicalConsultationReservations/${merchantOrderId}/pay`);
          ...
          <div className="bg-white p-6 rounded-xl shadow">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="mr-3">
                      ...
                      <p className="text-green-700 mt-1">
                        شكراً لك! تم تأكيد حجز استشارتك الطبية وسيتم التواصل معك قريباً.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="mr-3">
                      ...
                      <p className="text-yellow-700 mt-1">
                        يرجى إتمام عملية الدفع لتأكيد حجز استشارتك الطبية.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {doctor && (
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-bold text-gray-700 mb-2">معلومات الطبيب</h3>
            ...
            {/* Medical Complaint */}
            {reservation.medicalComplaint && (
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-bold text-gray-700 mb-2">الشكوى الطبية</h3>
                <p className="text-gray-900">{reservation.medicalComplaint}</p>
              </div>
            )}
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-3xl font-bold text-primary">{doctor?.reservationPrice || reservation.price || 0} $</span>
              </div>
            </div>
...
export default MedicalConsultationReservationsDetails;